...
from typing import Optional, Tuple, List, Optional, Set, Tuple, Union
...
from quantize.lr_norm import LRLayerNorm
...
class QuantViTLayer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.layernorm_before = LRLayerNorm(ori_layer.layernorm_before)
        self.layernorm_after = LRLayerNorm(ori_layer.layernorm_after)
    ...
    def smooth_and_quant_temporary(self):
        if self.let:
            ...
            # smooth_q_k_temporary(self.attention.attention.query, self.attention.attention.key,
            ...
            for name, module in self.named_modules():
                if isinstance(module, QuantLinear):
                    #cmodule.temp_weight = module.weight
            ...
            if isinstance(module, QuantLinear):
                if hasattr(module, "temp_weight"):
                    ...
                    if hasattr(self, f"{name_tmp}_smooth_rotate"):
                        ...
                        module.temp_weight = module.weight_quantizer(module.temp_weight, rotate=r)
                    else:
                    ...
                    if hasattr(self, f"{name_tmp}_smooth_rotate"):
                        r = getattr(self, f"{name_tmp}_smooth_rotate")
                        module.weight = module.weight_quantizer(module.weight, rotate=r)
                    else:
    ...
    def smooth_and_quant_inplace(self):
        if self.let:
            ...
            smooth_fc_fc_inplace(self.attention.attention.value, self.attention.output.dense,
                                self.fc1_smooth_scale, self.fc1_smooth_shift)
            # smooth_q_k_inplace(self.attention.attention.query, self.attention.attention.key,
            #                     self.qkt_smooth_scale)
        for name, module in self.named_modules():
            if isinstance(module, QuantLinear):
                name_tmp = name.replace(".","_")
    ...
    def lr_parameters(self, use_shift=True):
        params = []
        template = "smooth" if use_shift else "smooth_scale"
        for n, m in self.named_parameters():
